// lib/agents/matchAesthetic.ts
// Server-side helper: calls Claude Haiku to classify free-text aesthetic input
// into one of the known aesthetic IDs from aesthetics.json.
//
// Extracted from app/api/match-aesthetic/route.ts so it can be called
// server-side (e.g. from the Orchestrator) without an HTTP round-trip.
//
// The route keeps its own inline implementation so it can continue returning
// display names (e.g. "Quiet Structure") to the concept page UI.
// This function returns the slugified id (e.g. "quiet-structure").
...
/** Converts an aesthetic display name to the slugified id used in aesthetics.json. */
export function aestheticNameToId(name: string): string {
  return name.toLowerCase().replace(/\s+/g, '-');
}

/**
 * Calls Claude Haiku to match free-text aesthetic input to a known aesthetic id.
 * Returns null if no confident match or on LLM error.
 */
export async function matchAestheticId(input: string): Promise<string | null> {
...
Reply with ONLY the exact aesthetic name from the list, or the word "null" if nothing matches reasonably.`,
    ...
    const matched = AESTHETICS.find(a => a.toLowerCase() === raw.toLowerCase());
    return matched ? aestheticNameToId(matched) : null;
  } catch {
    return null;
  }
}
